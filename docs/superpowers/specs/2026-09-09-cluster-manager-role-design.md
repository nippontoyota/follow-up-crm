...
Add a latency-conscious, read-oriented `cluster_manager` role for exactly four hardcoded accounts. Cluster managers need visibility across their assigned branches, while Admin must not be able to create or edit additional cluster-manager accounts.
...
The Admin user form does not expose `cluster_manager`, and the users API rejects attempts to create that role. Cluster-manager requests use the cached branch-ID list with parameterized, indexed branch filters. No per-request membership joins or per-row authorization queries are introduced.
...
- Static checks confirm `cluster_manager` is absent from Admin creation options and rejected by the API.
